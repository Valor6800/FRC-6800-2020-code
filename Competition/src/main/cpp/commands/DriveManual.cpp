...
DriveManual::DriveManual(Drivetrain* drivetrain, std::function<double()> straightValue, std::function<double()> reverseValue, std::function<double()> turnValue, std::function<bool()> limelightInput) 
            : m_drivetrain{drivetrain}, m_straightValue{straightValue}, m_reverseValue{reverseValue}, m_turnValue{turnValue}, m_limelightInput{limelightInput} {
...
void DriveManual::Execute() {
    ...
    m_drivetrain->RocketLeagueDrive(m_straightValue(), m_reverseValue(), m_turnValue(), m_limelightInput());
}
